...
def main(config, mode, checkpoint):
    ...
    if config['dataset_parameters']['use_asl']:
        asl_train_path,  asl_test_path, asl_gloss2class, asl_class2gloss,asl_video2gloss = islr_datasets_loader(
            "ASL_Citizen")
    ...
    model = SkeletonTextCLIP(config["model"]).float().to(device)
...
if __name__ == "__main__":
    ...
    checkpoint ="/media/caffe/data_storage/CSLR/keyword_models/train/2026/0402/1756/99/model_epoch99.pth"  #model_epoch9.cpt or model_epoch9.pth or None
    # subprocess.run(command, input=("gazouken\n").encode(), check=True)
    # print("無効化完了")
    start = time.time()
