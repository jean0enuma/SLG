...
from trainer.Evaluation_trainer import EvaluationTrainer
from models.module.VAE_sync import VAE_sync
from models.evaluation.evaluation_model import EvaluationModel
...
def main(config, mode, vae_weights, checkpoint):
    ...
    tokenizer = gloss2class if is_islr else AutoTokenizer.from_pretrained(config['model']['tokenizer_name'])
    num_classes=len(gloss2class) if is_islr else tokenizer.vocab_size
    ...
    model = EvaluationModel(config['model']).to(device)
    ...
    trainer=EvaluationTrainer(config, scheduler)
    ...
    if mode == "train":
        if checkpoint != None and checkpoint.split(".")[-1] == "cpt":
            ...
            with open(f"{save_path}/wandb_id.txt", "r") as f:
                id = f.read()
            wandb.init(project="SLG_EVALUATION", id=id, resume="allow", entity="tkeda-jean-tokyo-city-university")
        else:
            wandb.init(project="SLG_EVALUATION", entity="tkeda-jean-tokyo-city-university")
            id = wandb.run.id
            # id名を保存
        ...
        trainer.fit(model, optimizer, scheduler, None, dl_train, dl_dev, dl_test, device,
                    early_stopping=None)
        if vae_weights!=None:
            vae_model=VAE_sync(config['model']['vae']).to(device)
            vae_model.load_state_dict(torch.load(vae_weights, map_location=device))
            tokenizer=AutoTokenizer.from_pretrained(config['model']['vae']['clip_text']['text_encoder_name'])
            ds_dev = SLGText2UnitsDatasets(dev_data_path, dev_cod_root, dev_face_root, trainable=False, is_3d=is_3d,
                                           is_processed=config['dataset_parameters']['is_processed'],
                                           is_sg_filter=False,
                                           is_coarse=False, tokenizer=tokenizer, texts_corpus=dev_corpus,
                                           is_islr=is_islr,gloss2class=gloss2class)
            ds_test = SLGText2UnitsDatasets(test_data_path, test_cod_root, test_face_root, trainable=False, is_3d=is_3d,
                                            is_processed=config['dataset_parameters']['is_processed'],
                                            is_sg_filter=False,
                                            is_coarse=False, tokenizer=tokenizer, texts_corpus=test_corpus,
                                            is_islr=is_islr,gloss2class=gloss2class)
            trainer.evaluation(model, vae_model, ds_dev, device)
            trainer.evaluation(model, vae_model, ds_test, device)
    elif mode=="eval":
        if vae_weights!=None:
            vae_model=VAE_sync(config['model']['vae']).to(device)
            vae_model.load_state_dict(torch.load(vae_weights, map_location=device))
            tokenizer=AutoTokenizer.from_pretrained(config['model']['vae']['model']['clip_text']['text_encoder_name'])
            ds_dev = SLGText2UnitsDatasets(dev_data_path, dev_cod_root, dev_face_root, trainable=False, is_3d=is_3d,
                                           is_processed=config['dataset_parameters']['is_processed'],
                                           is_sg_filter=False,
                                           is_coarse=False, tokenizer=tokenizer, texts_corpus=dev_corpus,
            ...
            ds_test = SLGText2UnitsDatasets(test_data_path, test_cod_root, test_face_root, trainable=False, is_3d=is_3d,
                                            ...
                                            is_coarse=False, tokenizer=tokenizer, texts_corpus=test_corpus,
                                            is_islr=is_islr, gloss2class=gloss2class)
            trainer.evaluation(model, vae_model, ds_dev, device, visualize_dir_name="visualize_devp")
            trainer.evaluation(model, vae_model, ds_test, device, visualize_dir_name="visualize_test")
...
if __name__ == "__main__":
    ...
    mode = "eval"  # Change this to "test" when you want to test
    vae_weights="/media/caffe/data_storage/CSLR/keyword_models/SLG/Evaluation_between_clip/textVAE_hand0.9/99/model_epoch99.pth"
    checkpoint ="/media/caffe/data_storage/CSLR/keyword_models/SLG/Evaluation_between_clip/back_translation/39/model_epoch39.pth"
    # subprocess.run(command, input=("gazouken\n").encode(), check=True)
    # print("無効化完了")
    start = time.time()
    print("Loading config...")
    with open(f"/home/caffe/work/SLG/Parameter/config_evaluation_islr.yaml", "r") as f:
        config = yaml.safe_load(f)
    config['model']['text_cond'] = False

    # logディレクトリにContinurous_Sign以下のディレクトリ，ファイルをコピー
    if checkpoint != None:
        if checkpoint.split(".")[-1] == "cpt" or mode=="eval":
            save_path = checkpoint.split("/")[:-2]
            save_path = "/".join(save_path)
            with open(f"{save_path}/config_evaluation_islr.yaml", "r") as f:
                config = yaml.safe_load(f)
        else:
            #再学習をかける時用
            old_path=checkpoint.split("/")[:-2]
            old_path="/".join(old_path)
            with open(f"{old_path}/config_evaluation_islr.yaml", "r") as f:
            ...
            #configをyamlファイルに保存
            with open(f"{save_path}/config_evaluation_islr.yaml", "w") as f:
                yaml.dump(config, f)
        ...
        log_create_dir(save_path)
        # copy_dir(PROJECT_DIR, save_path)
        shutil.copy(f"/home/caffe/work/SLG/Parameter/config_evaluation_islr.yaml", f"{save_path}/config_evaluation_islr.yaml")
        copy_dir(PROJECT_DIR, save_path)
        shutil.rmtree(f"{save_path}/Continurous_Sign/wandb")  # wandbディレクトリはコピー後に削除(ログが混ざるのを防ぐため)
    if vae_weights != None:
        with open(f"{'/'.join(vae_weights.split('/')[:-2])}/config_sync.yaml", "r") as f:
            vae_config = yaml.safe_load(f)
        config['model']['vae']=vae_config
    print("Config loaded.")
