...
def main(config, mode, checkpoint):
    ...
    model =MBartText2Pose(config['model']).to(device)
    ...
    if mode == "train":
        # configのセーブ
        with open(f"{save_path}/config_mbart.yaml", "w") as f:
            yaml.dump(config, f)
        if checkpoint != None and checkpoint.split(".")[-1] == "cpt":
            # id名を取得
...
if __name__ == "__main__":
    ...
    vqvae_weights = "/media/caffe/data_storage/CSLR/keyword_models/train/2026/0518/1224/19/model_epoch19.pth"
    checkpoint =None
    # subprocess.run(command, input=("gazouken\n").encode(), check=True)
    # print("無効化完了")
    start = time.time()
    print("Loading config...")
    with open(f"/home/caffe/work/SLG/Parameter/config_mbart.yaml", "r") as f:
    ...
    if checkpoint != None:
        if checkpoint.split(".")[-1] == "cpt" or mode=="visualize":
            ...
            with open(f"{save_path}/config_mbart.yaml", "r") as f:
                config = yaml.safe_load(f)
        else:
            #再学習をかける時用
            old_path=checkpoint.split("/")[:-2]
            old_path="/".join(old_path)
            with open(f"{old_path}/config_mbart.yaml", "r") as f:
            ...
            #shutil.copy(f"/home/caffe/work/SLG/Parameter/config_mbart.yaml.yaml", f"{save_path}/config_mbart.yaml.yaml")
            #configをyamlファイルに保存
            with open(f"{save_path}/config_mbart.yaml", "w") as f:
                yaml.dump(config, f)
        ...
        log_create_dir(save_path)
        # copy_dir(PROJECT_DIR, save_path)
        shutil.copy(f"/home/caffe/work/SLG/Parameter/config_mbart.yaml", f"{save_path}/config_mbart.yaml")
        copy_dir(PROJECT_DIR, save_path)
        shutil.rmtree(f"{save_path}/Continurous_Sign/wandb")  # wandbディレクトリはコピー後に削除(ログが混ざるのを防ぐため)
