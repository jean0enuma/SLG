            ...
            if self.is_3d:
                try:
                    if not self.is_openpose:
                        ...
                        B,JC=cod_data.shape
                        B,JF=face_data.shape
                        cod_data=cod_data.reshape(B,-1,3).transpose(2,0,1)#(3,T,JC)
                        face_data=face_data.reshape(B,-1,2).transpose(2,0,1)#(2,T,FC)
                ...
                cod_data, face_cod_data, hand_cod_data, body_cod_data=coordinate_preprocess(cod_data,
                                                                                            face_data,
                                                                                            is_face_connect=True,
                                                                                            is_sg_filter=False,is_openpose=self.is_openpose)
            # face_cod_data=coordinate_preprocess_face(face_data)
            C,T,J = cod_data.shape
            cod_data = torch.tensor(cod_data)  # (3,T,JC)
            #if self.trainable==False:
            cod_data = self.temporal_rescale(cod_data.permute(1,2,0).reshape(T,-1)).reshape(-1,J,C).permute(2,0,1) #(3,T,JC)
            if self.is_norm:
            ...
            if self.is_islr==True:
                ...
                if self.return_length:
                    if self.gloss2class is not None:
                        if self.is_islr:
                            input_ids=torch.tensor([self.gloss2class[sequence.lower()]],dtype=torch.long)
                            attention_mask=torch.tensor([1],dtype=torch.long)
                            sequence_token=BatchEncoding(
                                {"input_ids": input_ids,
                                 "attention_mask": attention_mask}
                            )
                        else:
                            input_ids=torch.tensor([self.gloss2class[tokens] for tokens in sequence.split(" ")],dtype=torch.long)
                            attention_mask=torch.tensor([1] * len(sequence.split(" ")),dtype=torch.long)
                            sequence_token=BatchEncoding(
                                {"input_ids": input_ids,
                                 "attention_mask": attention_mask}
                            )
                    else:
                        sequence_token=self.tokenizer([sequence], padding=False, truncation=True, return_tensors='pt')
                    #if self.gloss2class is not None:
                    #    n_class=self.gloss2class[sequence]
                    #    return pose_data,hand_mask,input_length,id,data_path,sequence_token,n_class,center_data,shoulder_length,left_center_data,left_length,right_center_data,right_length
                    return pose_data,hand_mask,input_length,id,data_path,sequence_token,center_data,shoulder_length,left_center_data,left_length,right_center_data,right_length
                ...
                if self.return_length:
                    if self.gloss2class:
                        input_ids = torch.tensor([self.gloss2class[tokens] for tokens in sequence.split(" ")])
                        attention_mask=torch.tensor([1] * len(sequence.split(" ")), dtype=torch.long)
            ...
            if isinstance(self.tokenizer, dict):
                ...
                if self.is_islr:
                    ...
                    input_ids = torch.tensor(
                        [self.tokenizer[tokens.lower()] for tokens in sentence_list],
                        dtype=torch.long,
                    )
                    attention_mask=torch.tensor([1] * len(sentence_list),dtype=torch.long)
                else:
                    eos_token=self.tokenizer["eos_token"]
                    bos_token=self.tokenizer["bos_token"]
                    new_sentence_list=[f"bos_token {sentence} eos_token" for sentence in sentence_list]
