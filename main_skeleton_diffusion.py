...
import time
from models.module.skeleton_text_lddm import SkeletonTextDiffusion,ModelConfig
...
def main(config, mode, vae_weights, checkpoint):
    ...
    if config['dataset_parameters']['use_csl-daily']:
        csl_daily_train_path, csl_daily_dev_path, csl_daily_test_path, csl_daily_gloss2class, csl_daily_class2gloss, csl_daily_video2gloss= islr_datasets_loader(
            "CSL-Daily")

        train_corpus[1] = csl_daily_gloss2class
        dev_corpus[1] = csl_daily_class2gloss
        test_corpus[1] = csl_daily_video2gloss
        um_class = len(csl_daily_gloss2class)
        train_cod_root[3] = f"{WORDS_DATADIR_CSL_DAILY_SKELETON}/train"
        dev_cod_root[3] = f"{WORDS_DATADIR_CSL_DAILY_SKELETON}/dev"
        test_cod_root[3] = f"{WORDS_DATADIR_CSL_DAILY_SKELETON}/test"

        train_face_root[3] = f"{WORDS_DATADIR_CSL_DAILY_SKELETON_FACE}/train"
        dev_face_root[3] = f"{WORDS_DATADIR_CSL_DAILY_SKELETON_FACE}/dev"
        test_face_root[3] = f"{WORDS_DATADIR_CSL_DAILY_SKELETON_FACE}/test"
        is_3d = True

        train_data_path += integrate_path(3, csl_daily_train_path)
        dev_data_path += integrate_path(3, csl_daily_dev_path)
        test_data_path += integrate_path(3, csl_daily_test_path)
        is_islr = True
        i += 1
    ...
    if config['dataset_parameters']['use_phoenix']:
        if config['dataset_parameters']['is_islr']:
            ...
            gloss2class=phoenix_gloss2class
    ...
    if config['dataset_parameters']['use_asl']:
        asl_train_path,asl_dev_path,  asl_test_path, asl_gloss2class, asl_class2gloss,asl_video2gloss= islr_datasets_loader(
            "ASL_Citizen",train_test_split="split")
        train_corpus[4] =  asl_video2gloss
        dev_corpus[4] =  asl_video2gloss
        test_corpus[4] = asl_video2gloss
        if config['dataset_parameters']['is_processed']:
    ...
    tokenizer['pad_token'] = len(gloss2class)
    ...
    ds_train = SLGText2UnitsDatasets(train_data_path, train_cod_root, train_face_root, is_3d=is_3d,
                                     ...
                                     scale_ratio=(1.5,1.8),gloss2class=gloss2class)
    ...
    ds_test = SLGText2UnitsDatasets(test_data_path, test_cod_root, test_face_root, trainable=False, is_3d=is_3d,
                                    ...
                                    scale_ratio=(0.8,1.2),gloss2class=gloss2class)

    if ds_train.is_3d:
        config['model']['skeleton_dim']=int(config['model']['skeleton_dim']*3)# 3Dの場合の出力サイズ
        config['model']['skeleton_hand_dim']=int(config['model']['skeleton_hand_dim']*3)# 3Dの場合の出力サイズ
        config['model']['skeleton_body_dim']=int(config['model']['skeleton_body_dim']*3)# 3Dの場合の出力サイズ
    else:
        config['model']['skeleton_dim']=int(config['model']['skeleton_dim']*2)# 3Dの場合の出力サイズ
        config['model']['skeleton_hand_dim']=int(config['model']['skeleton_hand_dim']*2)# 3Dの場合の出力サイズ
        config['model']['skeleton_body_dim']=int(config['model']['skeleton_body_dim']*2)# 3Dの場合の出力サイズ    postfix = ""
    if config["dataset_parameters"]["use_phoenixT"]:
        postfix = "_phoenixT"
    if config["dataset_parameters"]["use_csl-daily"]:
        postfix += "_csl_daily"
    ...
    model_cfg = ModelConfig(
        ...
        skeleton_dim=config['model']['skeleton_dim'], skeleton_hand_dim=config['model']['skeleton_hand_dim'], skeleton_body_dim=config['model']['skeleton_body_dim'],
        max_seq_len=config['model']['max_seq_len'], latent_dim=config['model']['latent_dim'],
        d_model=config['model']['d_model'], nhead=config['model']['nhead'], num_layers=config['model']['num_layers'], dim_feedforward=config['model']['dim_feedforward'],
        num_timesteps=config['model']['num_timesteps'],mode=config['model']['mode'],kl_weight=config['model']['kl_weight'],
        use_latent_mean=config['model']['use_latent_mean'],cfg_dropout=config['model']['cfg_dropout'],guidance_scale=config['model']['guidance_scale'],
        text_encoder_type=config['model']['text_encoder_type'],text_num_layers=config['model']['text_num_layers'],text_nhead=config['model']['text_nhead'],text_dim_feedforward=config['model']['text_dim_feedforward'],
    )
    model = SkeletonTextDiffusion(model_cfg).to(device)
    if vae_weights!=None:
        model.load_vae(vae_weights, device)
        model.compute_latent_stats(dl_train, device,per_dim=True,standardize=True)
    ...
    if config['lr_parameters']['scheduler_type'] == "CosineAnnealingLR":
        ...
        if config['lr_parameters']['scheduler_timing'] == "epoch":
            ...
            scheduler = CosineAnnealingLR(optimizer, warmup_epochs=int(len(dl_train) * config["lr_parameters"]["epoch"] * 0.1),
    ...
    trainer=DiffusionTrainer(config,scheduler)
    ...
    if mode == "train":
        ...
        trainer.fit(model, optimizer, scheduler, dl_train, dl_dev, dl_test, device,
                    early_stopping=None)
        trainer.visualize(model, ds_dev, device,visualize_dir_name="visualize_dev")
        trainer.visualize(model, ds_test, device,visualize_dir_name="visualize_test")
    elif mode == "visualize":
        trainer.visualize(model, ds_dev, device, visualize_dir_name="visualize_dev")
        trainer.visualize(model, ds_test, device, visualize_dir_name="visualize_test")
    else:
...
if __name__ == "__main__":
    ...
    #vae_weights="/media/caffe/data_storage/CSLR/keyword_models/train/2026/0605/1919/19/model_epoch19.pth"
    #vae_weights="/media/caffe/data_storage/CSLR/keyword_models/train/2026/0604/1721/19/model_epoch19.pth"
    #vae_weights="/media/caffe/data_storage/CSLR/keyword_models/train/2026/0608/1510/19/model_epoch19.pth"
    #vae_weights="/media/caffe/data_storage/CSLR/keyword_models/train/2026/0612/1058/6/model_epoch6.pth"
    vae_weights=None
    checkpoint =None
    # subprocess.run(command, input=("gazouken\n").encode(), check=True)
    # print("無効化完了")
    ...
    with open(f"/home/caffe/work/SLG/Parameter/config_skeleton_diffusion.yaml", "r") as f:
    ...
    if checkpoint != None:
        if checkpoint.split(".")[-1] == "cpt" or mode=="visualize":
            save_path = checkpoint.split("/")[:-2]
            save_path = "/".join(save_path)
            with open(f"{save_path}/config_skeleton_diffusion.yaml", "r") as f:
                config = yaml.safe_load(f)
        else:
            #再学習をかける時用
            old_path=checkpoint.split("/")[:-2]
            old_path="/".join(old_path)
            with open(f"{old_path}/config_sync.yaml", "r") as f:
                config_old = yaml.safe_load(f)
            config['model']=config_old['model']
            while True:
            ...
            #configをyamlファイルに保存
            with open(f"{save_path}/config_sync.yaml", "w") as f:
                yaml.dump(config, f)
        ...
        log_create_dir(save_path)
        # copy_dir(PROJECT_DIR, save_path)
        shutil.copy(f"/home/caffe/work/SLG/Parameter/config_skeleton_diffusion.yaml", f"{save_path}/config_skeleton_diffusion.yaml")
        copy_dir(PROJECT_DIR, save_path)
        shutil.rmtree(f"{save_path}/Continurous_Sign/wandb")  # wandbディレクトリはコピー後に削除(ログが混ざるのを防ぐため)
