...
def visualize_skeletons(predicted_d6,gt_cods,data_path, lengths, save_dir, bones, mask=None,):
    ...
    pred_coordinates = reconstruct_joints_from_6d(gt_cods,predicted_d6, bones=bones, bone_mask=mask)
...
def main(config, dataset,save_path,parts="hands",visualize=False,used_3d=False,vae_weights=None,weights=None):
    ...
    vae=HandTransformerVAE(in_channels=6,bones=bones,n_stages=vae_config['n_stages'],blocks_per_stage=vae_config['blocks_per_stage'],
                            dropout=vae_config['dropout'],d_model=vae_config['d_model'],latent_dim=vae_config['latent_dim'])
    ...
    model=SignLatentFlowMoE(vae,hand_vae=hand_vae,body_vae=body_vae,
                            boundary=model_config['boundary'],
                              ...
                              cond_dim=model_config['cond_dim'],cross_attn=True)   # テキスト埋め込み次元 (例: T5系)

    with open(f"{save_path}/flow_match.yaml", "w") as f:
        yaml.dump(config, f)

    text_encoder=AutoModel.from_pretrained("google-bert/bert-base-multilingual-uncased")
    text_encoder.eval()
    ...
    if visualize!=True:
            ...
            for batch in dl_dev:
                ...
                with torch.no_grad():
                    ...
                    loss, logs = model.training_loss(
                        ...
                        part_weight={"body": 1.0, "left": 2.0, "right": 2.0},cond_mask=context_mask)
            ...
            if step>5000:
                ...
                for batch in dl_dev:
                    ...
                    with torch.no_grad():
                        ...
                        pred_cods=ema.ema_model.sample(1,input_length_tensor[idx], lengths=input_length_tensor[idx:idx+1], cond=text_emb[idx:idx+1],cond_mask=context_mask[idx:idx+1],decode="parts",guidance_scale=3.0)
        ...
        print("ema_model training:", ema.ema_model.training,
              " vae training:", ema.ema_model.vae.training,
              " hand_vae:", ema.ema_model.hand_vae.training)
...
if __name__ == "__main__":
    ...
    vae_weights="/media/caffe/data_storage/CSLR/keyword_models/FlowMatching/results"
    with open(f"/home/caffe/work/SLG/Parameter/config_flowmatch.yaml", "r") as f:
        config = yaml.safe_load(f)
    if os.path.exists(f"{os.path.dirname(vae_weights)}/config_vae.yaml"):
        with open(f"{os.path.dirname(vae_weights)}/config_vae.yaml", "r") as f:
