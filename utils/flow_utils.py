...
def compute_color(u, v):
    ...
    rad = np.sqrt(u ** 2 + v ** 2)
    rad_max = np.max(rad)  # Normalize by the maximum radius
    if rad_max > 0:
        u = u / rad_max
        v = v / rad_max
        rad = rad / rad_max
    ...
    #normalize angle
    #a=(a-np.average(a))/(np.std(a)+1e-10)
    fk = (a + 1) / 2 * (ncols - 1) + 1
...
if __name__=="__main__":
    ...
    u = -y
    v = -x
    # Compute normalized optical flow visualization
    normalized_flow_image = compute_color(u, v)

    # Display the result
    plt.figure(figsize=(6, 6))
    plt.imshow(normalized_flow_image)
    plt.axis('off')  # Remove axes
    plt.tight_layout(pad=0)  # Remove extra padding
    plt.savefig("/home/jean/work/MAE_csr/optical_flowmap.png")