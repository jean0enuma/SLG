...
def ctcdecode_WER(file_path,class2gloss,special_token:dict,auxiliary_pred=False,dataset="phoenix"):
    if type(file_path)==str:
        output=np.load(file_path,allow_pickle=True).item()
    elif type(file_path)!=str:
        output=file_path
    vocab =[x for x in class2gloss.values()]
    special_token_idx=special_token.keys()
    #ctc_decoder=ctcdecode.CTCBeamDecoder(vocab,beam_width=10,blank_id=0,num_processes=10)
    #ctc_decoder2=RNNTBeamSearch(vocab,beam_size=10,blank=0)
    #ctc_decoder2=ctc_decoder(lexicon=None,tokens=vocab,beam_size=10,blank_token="<blank>",sil_token="<sep>",unk_word="<pad>")
    ...
    WER_calculatorALL=WordErrorRate()
    wer_dict={}
    wer_list=[]
    gloss_sentence_dict={}
    for k ,v in output.items():
        ...
        if "conv_feature"  in v.keys():
            ...
            conv_hyp=[]
        if "all_feature" in v.keys() and v["all_feature"] is not None:
            all_feature=torch.tensor(v["all_feature"]).unsqueeze(0)
            all_beam_result,beam_scores,timesteps,out_seq_len=ctc_decoder2.decode(all_feature.float())
            if len(all_beam_result[0])!=0:
                all_first_result = all_beam_result[0][0][:out_seq_len[0][0]].cpu().numpy()
                all_hyp = class2gloss_sequence(dataset,all_first_result, class2gloss,special_token)
            else:
                all_hyp=[]
        else:
            all_hyp=[]
        ...
        if "all_feature" in v.keys() and v["all_feature"] is not None:
            WER_calculatorALL.update(" ".join(all_hyp), " ".join(ref))
        if len(hyp)==0:
            continue
    ...
    if "all_feature" in v.keys() and v["all_feature"] is not None:
        retALL=WER_calculatorALL.compute()*100
        print(f"WERALL:{retALL.item()}%")
    return ret
