...
from SLG_datasets.SLG_datasets_notokenizer import SLGText2UnitsDatasetsNoTokenizer
from trainer.Mbart_trainer import MbartTrainer
from models.module.ScratchDecoder import FrozenEncoderDecoder
...
def main(config, mode, checkpoint):
    ...
    ds_train = SLGText2UnitsDatasetsNoTokenizer(train_data_path, train_cod_root, train_face_root, is_3d=is_3d,
                                     is_processed=config['dataset_parameters']['is_processed'], is_sg_filter=False,
                                     is_coarse=False,
                                     trainable=True,texts_corpus=train_corpus)
    ds_dev = SLGText2UnitsDatasetsNoTokenizer(dev_data_path, dev_cod_root, dev_face_root, trainable=False, is_3d=is_3d,
                                   is_processed=config['dataset_parameters']['is_processed'], is_sg_filter=False,
                                   is_coarse=False,texts_corpus=dev_corpus)
    ds_test = SLGText2UnitsDatasetsNoTokenizer(test_data_path, test_cod_root, test_face_root, trainable=False, is_3d=is_3d,
                                    is_processed=config['dataset_parameters']['is_processed'], is_sg_filter=False,
                                    is_coarse=False,texts_corpus=test_corpus)

    if ds_train.is_3d:
        config['model']['pose_dim']=int(config['model']['pose_dim']*1.5)# 3Dの場合の出力サイズ
        config['model']['vqvae_config']['model']['pose_dim']=int(config['model']['vqvae_config']['model']['pose_dim']*1.5)
        config['model']['vqvae_config']['model']['in_dim']=int(config['model']['vqvae_config']['model']['in_dim']*1.5)
    postfix = ""
    if config["dataset_parameters"]["use_phoenixT"]:
        postfix = "_phoenixT"
    if config["dataset_parameters"]["use_csl-daily"]:
    ...
    model =FrozenEncoderDecoder(config['model']).to(device)
    model.pose_tokenizer.load_state_dict(torch.load(vqvae_weights, map_location=device), strict=False)
    ...
    trainer=MbartTrainer(config, scheduler)
    ...
    if mode == "train":
        if checkpoint != None and checkpoint.split(".")[-1] == "cpt":
            ...
            with open(f"{save_path}/wandb_id.txt", "r") as f:
                id = f.read()
            wandb.init(project="SLG_MBART", id=id, resume="allow", entity="tkeda-jean-tokyo-city-university")
        else:
            wandb.init(project="SLG_MBART", entity="tkeda-jean-tokyo-city-university")
            id = wandb.run.id
            # id名を保存
...
if __name__ == "__main__":
    ...
    vqvae_weights = "/media/caffe/data_storage/CSLR/keyword_models/SLG/VQVAE/FSQ_n_codes64/19/model_epoch19.pth"
    checkpoint =None
    # subprocess.run(command, input=("gazouken\n").encode(), check=True)
    # print("無効化完了")
    start = time.time()
    print("Loading config...")
    with open(f"/home/caffe/work/SLG/Parameter/config_encdec.yaml", "r") as f:
    ...
    if checkpoint != None:
        if checkpoint.split(".")[-1] == "cpt" or mode=="visualize":
            ...
            with open(f"{save_path}/config_encdec.yaml", "r") as f:
                config = yaml.safe_load(f)
        else:
            #再学習をかける時用
            old_path=checkpoint.split("/")[:-2]
            old_path="/".join(old_path)
            with open(f"{old_path}/config_encdec.yaml", "r") as f:
            ...
            #shutil.copy(f"/home/caffe/work/SLG/Parameter/config_encdec.yaml.yaml", f"{save_path}/config_encdec.yaml.yaml")
            #configをyamlファイルに保存
            with open(f"{save_path}/config_encdec.yaml", "w") as f:
                yaml.dump(config, f)
        ...
        log_create_dir(save_path)
        # copy_dir(PROJECT_DIR, save_path)
        shutil.copy(f"/home/caffe/work/SLG/Parameter/config_encdec.yaml", f"{save_path}/config_encdec.yaml")
        copy_dir(PROJECT_DIR, save_path)
        shutil.rmtree(f"{save_path}/Continurous_Sign/wandb")  # wandbディレクトリはコピー後に削除(ログが混ざるのを防ぐため)

    config['save_path'] = save_path
    if vqvae_weights!=None:
        vqvae_path=vqvae_weights.split("/")[:-2]
        vqvae_path="/".join(vqvae_path)
        with open(f"{vqvae_path}/config_vqvae.yaml", "r") as f:
            vqvae_config = yaml.safe_load(f)
        config['model']['vqvae_config']=vqvae_config
