...
class MbartTrainer:
    def __init__(self,config,scheduler=None):
        ...
        self.step=0
        self.accumulate_steps=self.config["lr_parameters"]["accumulate_step"] if "accumulate_step" in self.config["lr_parameters"].keys() else None
    ...
    def train(self, model, optimizer, criterion, train_loader, device,ema=False):
        ...
        for batch_idx, batch in tqdm(enumerate(train_loader), total=len(train_loader.dataset) // train_loader.batch_size):
            ...
            with torch.cuda.amp.autocast(dtype=torch.bfloat16,enabled=self.config["lr_parameters"]["amp"]):
                output = model(sequence,padded_cod_data,input_length_tensor)


                loss = output['loss']/ self.config["lr_parameters"]["accumulate_step"] if self.accumulate_steps is not None else output['loss']
            ...
            if batch_idx % 100== 0:
                tqdm.write(f"Avg Loss: {np.mean(total_loss)*self.config['lr_parameters']['accumulate_step'] if self.accumulate_steps is not None else np.mean(total_loss):.4f}")


        avg_loss = np.mean(total_loss).astype(np.float32)*self.config["lr_parameters"]["accumulate_step"] if self.accumulate_steps is not None else np.mean(total_loss).astype(np.float32)
    ...
    def visualize(self, model, dataset, device,visualize_dir_name="visualize"):
        ...
        with torch.no_grad():
            for batch in tqdm(dataset, total=len(dataset)):
                ...
                output = model.sample(sequence,input_length_tensor)
