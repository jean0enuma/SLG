...
if __name__ == "__main__":
    ...
    mode = "visualize"
    checkpoint ="/media/caffe/data_storage/CSLR/keyword_models/train/2026/0227/1211/99/model_epoch99.pth"
    # subprocess.run(command, input=("gazouken\n").encode(), check=True)
    # print("無効化完了")
    start = time.time()
    print("Loading config...")
    with open(f"/home/caffe/work/SLG/Parameter/config.yaml", "r") as f:
    ...
    if checkpoint != None:
        ...
        with open(f"{save_path}/config.yaml", "r") as f:
            config = yaml.safe_load(f)
    else:
        save_path = copy.deepcopy(SAVE_MODEL_DIR_KWT)
        log_create_dir(save_path)
        # copy_dir(PROJECT_DIR, save_path)
        shutil.copy(f"/home/caffe/work/SLG/Parameter/config.yaml", f"{save_path}/config.yaml")
