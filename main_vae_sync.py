...
def main(config, mode, clip_weights, checkpoint):
    ...
    if config['model']['clip_text']['text_encoder_name'] =="unum-cloud/uform-vl-multilingual-v2":
        import uform

        _, tokenizer = uform.get_model('unum-cloud/uform-vl-multilingual-v2')
    #tokenizer = AutoTokenizer.from_pretrained(config['model']['clip_text']['text_encoder_name'])
    ...
    config['model']['clip_text']['vocab_size']=len(tokenizer)
    model = VAE_sync(config).to(device)
    #model.mean_anchor_frame(dl_train)
    #modelのmodel.diffusion以外の重みをvae_weightsから読み込む
    if clip_weights != None:
        model_dict = model.state_dict()
        vae_weights_dict = torch.load(clip_weights, map_location=device)
        # diffusionの重みを除外して読み込む
        model.clip_load(vae_weights_dict)
...
if __name__ == "__main__":
    ...
    if clip_weights != None:
        with open(f"{'/'.join(clip_weights.split('/')[:-2])}/config_sync.yaml", "r") as f:
            vae_config = yaml.safe_load(f)
        config['model']['clip_text']=vae_config['model']['clip_text']
    print("Config loaded.")
    ...
    main(config, mode, clip_weights=clip_weights, checkpoint=checkpoint)
