...
from models.module.VAE_class_embed import VAE_class_embed
...
def main(config, mode, clip_weights, checkpoint):
    ...
    model = VAE_class_embed(config).to(device)
    #model.create_class_embed(ds_train, tokenizer, device, save_path=save_path)
    model.load_class_embed("/home/caffe/work/SLG/Parameter/class_embed.npy", device)
...
if __name__ == "__main__":
    ...
    clip_weights="/media/caffe/data_storage/CSLR/keyword_models/SLG/skeleton_clip_asl_citizen/88/model_epoch88.pth"
    checkpoint =None
    # subprocess.run(command, input=("gazouken\n").encode(), check=True)
    # print("無効化完了")
    start = time.time()
