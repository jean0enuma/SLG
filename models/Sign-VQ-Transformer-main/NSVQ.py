...
class NSVQ(torch.nn.Module):
    def __init__(
        ...
        replace: bool = False,
        replace_epoch: list = None,
        call_every: list = None,
        mode: str = "codebook",
    ...
    def forward(self, input_data):
        ...
        self.perplexity = torch.exp(
            -torch.sum(avg_probs * torch.log(avg_probs + self.eps))
        )
        ...
        return quantized_input, hard_quantized_input.detach()

    def replace_unused_codebooks(self, num_batches, encoder_predictions):
        ...
        Recommendation: Call the function more frequently for the initial stages of training to capture a useful set of
        codebook entries soon. Then call it less frequently until the end of training to make sure you discard inactive
        codebook entries. Finally, stop calling the function at the latest stages of training in order not to introduce
        new codebook entries which would not have the right time to be tuned and optimized until the end of training.
        ...
        {50,100,200,...}. For instance, as a commonly used case, if we set the self.discard_threshold=0.01 and
        ...
        with torch.no_grad():
            ...
            if used_count == 0:
                print(
                    f"####### used_indices equals zero / shuffling whole codebooks ######"
                )
                self.codebooks += (
                    self.eps
                ...
                if self.mode == "codebook":
                    ...
                    self.codebooks[unused_indices] += (
                        encoder_predictions[
                            torch.randint(
                                low=0,
                                high=encoder_predictions.shape[0],
                                size=(unused_count,),
                            )
                        ]
                        .clone()
                        .to(self.codebooks.device)
                    )
                elif self.mode == "both":
                    if self.replacement_called % 2 == 0:
                        # codebook
                        self.codebooks[unused_indices] += (
                            used_codebooks[range(unused_count)]
                            + self.eps
                        ...
                        # encoder
                        self.codebooks[unused_indices] += encoder_predictions[
                            torch.randint(
                                low=0,
                                high=encoder_predictions.shape[0],
                                size=(unused_count,),
            ...
            [
                logger.log_metrics(
                    {"Codebook Replaced": round(unused_count / self.num_embeddings, 4)}
                )
                for logger in self.loggers.values()
            ]

            self.codebooks_used[:] = 0.0
            self.replacement_called += 1
            return unused_count

    def run_replacement(self, epoch, batches, encoder_predictions):
        removed = 0
        if self.replace:
            if (
                self.phase >= (len(self.replace_epoch) - 1)
                or self.replace_epoch[0] > epoch
            ):
                return removed, batches

            if self.replace_epoch[self.phase + 1] <= epoch:
                self.phase += 1

            # codebook replacement
            if self.call_every[self.phase] != -1:
                if epoch % self.call_every[self.phase] == 0:
                    removed = self.replace_unused_codebooks(
                        batches, encoder_predictions
                    )
                    batches = 0
        else:
            self.codebooks_used[:] = 0.0
    ...
    def query_codebook(self, inputs):
        # compute the distances between input and codebooks vectors
        # distances = (
        #         torch.sum(input_data ** 2, dim=1, keepdim=True)
        #         - 2 * (torch.matmul(input_data, self.codebooks.t()))
        ...
        # hard_quantized_input = self.codebooks[min_indices]
        with torch.no_grad():
            embedding_size = self.codebooks.size(1)
            inputs_size = inputs.size()
            inputs_flatten = inputs.view(-1, embedding_size)

            codebook_sqr = torch.sum(self.codebooks ** 2, dim=1)
            inputs_sqr = torch.sum(inputs_flatten ** 2, dim=1, keepdim=True)
    ...
    def inference(self, input_data):
        ...
        if self.weighting == True:
            weights_normalized = (
                (self.weights / (torch.linalg.vector_norm(self.weights) + self.eps))
                .detach()
                .clone()
            )

            weighted_input = input_data * weights_normalized
            weighted_codebooks = codebooks * weights_normalized

            distances = (
                torch.sum(weighted_input ** 2, dim=1, keepdim=True)
                - 2 * (torch.matmul(weighted_input, weighted_codebooks.t()))
                + torch.sum(weighted_codebooks.t() ** 2, dim=0, keepdim=True)
            )
